...
exec env -i -- /bin/bash -c "source /etc/profile; export HOME=`pwd`; export PATH='/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'; exec buildbot-worker start --nodaemon worker"
